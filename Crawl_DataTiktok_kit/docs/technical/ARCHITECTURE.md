...
| `script-filter.cjs` | **Lọc tiêu đề theo ngôn ngữ**: cho phép tiếng Anh + ngôn ngữ của **chính quốc gia profile** (nhãn `(US)`/`(KR)`…). Hai dấu hiệu độc lập: tiền tố tên sound (= ngôn ngữ người đăng) và hệ chữ Unicode ([QĐ-41](DECISIONS.md)) |
| `crawler/not-interested.cjs` | Bấm ⋯ → **Not interested** cho video vừa bị loại vì ngôn ngữ. ⚠ Tuyệt đối không bấm theo toạ độ, loại thẳng mọi phần tử có chữ `report` — nút Report nằm ngay dưới ([QĐ-41](DECISIONS.md)) |
...
| `lang-filter.test.js` | **52 khẳng định** cho bộ lọc ngôn ngữ + bấm Not interested (QĐ-41). Kiểm theo **từng quốc gia**: `(US)` loại tiếng Hàn, `(KR)` lấy tiếng Hàn nhưng loại tiếng Nhật, `(JP)` ngược lại; Ả Rập/Devanagari loại ở mọi nơi; emoji **không** bị tính là chữ lạ. Phần bấm menu chạy **thật trong Chromium**.<br>⚠ Mục 7b dựng menu có **khối bọc chứa cả "Not interested" lẫn "Report"** — đó là cảnh **duy nhất** làm chốt tránh-Report có tác dụng; thiếu nó thì bỏ hẳn chốt mà test vẫn pass (đã bị lừa một lần) |
...
| **Bỏ cuộc khi bị chặn KÉO DÀI** | **6** lần lỗi liên tiếp → phát `count-blocked` → **dừng profile đó rồi tự bật lại 5/15/30 phút**. Trước đây backoff **không có điểm dừng**: 1 sound mỗi ~6 phút suốt đêm, vòng quét đứng hẳn, đo thật 40 phút ra **0 sound hữu ích** ([QĐ-35](DECISIONS.md)) |
